...
st.set_page_config(page_title="Ren — LINE Team", page_icon="🟠", layout="wide", initial_sidebar_state="collapsed")
...
with tab_team:
    render_kanban(tasks, TEAM_COLUMNS, members, key_prefix="rt_", use_owner=True)

with tab_me:
    my_tasks = get_personal_tasks(tasks, MY_NAME)
    active = len([t for t in my_tasks if t["column"] != "done"])
    st.caption(f"アクティブ {active}件")
    render_personal_kanban(tasks, MY_NAME, members, key_prefix="rm_")
